/*
 add-credit-jiacomolusso-20.js — idempotent startup helper to credit $20 to jiacomolusso@yahoo.com (withdrawable)
...
(async function creditJiaco20(){
  try{
    const TARGET_EMAIL = 'jiacomolusso@yahoo.com';
    const AMOUNT = 20;
    const FLAG_KEY = 'CUP9_SYSTEM_CREDIT_jiaco_20_APPLIED';
      ...
      try{
        ...
        return txs.find(t=>{
          try{
            ...
            const isFlag = t.meta && (t.meta._system_credit_jiaco_20 === true || (t.meta && t.meta._system_credit_key === 'jiaco_20'));
    ...
    if(API_BASE){
      try{
        ...
        const resp = await fetch(url, {
          method: 'POST',
          headers,
          body: JSON.stringify({ email: TARGET_EMAIL, amount: AMOUNT, reason: 'system-credit-jiaco-20' }),
        });
        ...
        if(resp.ok){
          try{
            ...
            if(body && body.transaction && body.transaction.id){
              ...
              if(!txs.find(x=>String(x.id) === String(body.transaction.id))){
                ...
                tx.meta = tx.meta || {};
                tx.meta._system_credit_jiaco_20 = true;
                tx.meta._system_credit_key = 'jiaco_20';
              ...
              // Ensure CUP9_EARNINGS reflects backend credit locally for UI continuity
              try{
          ...
          console.warn('add-credit-jiaco-20 backend failed', resp.status, text);
          try{ toastMessage(`Accreditamento backend fallito: ${resp.status}`, { type:'error' }); }catch(e){}
        }
      }catch(e){
        console.warn('add-credit-jiaco-20 backend request failed', e);
    ...
    try{
      // durable flag guard
      try{ if(localStorage.getItem(FLAG_KEY) === '1'){ console.info('add-credit-jiaco-20: already applied (flag)'); return; } }catch(e){}

      if(existingAccreditedTxLocal()){
        console.info('add-credit-jiaco-20: accredited tx already exists locally; skipping local credit');
        try{ localStorage.setItem(FLAG_KEY, '1'); }catch(e){}
        return;
      }
      ...
      const tx = {
        ...
        txhash: 'system-jiaco-20-' + txId,
        created_at: nowIso,
        status: 'accredited',
        email: String(TARGET_EMAIL).toLowerCase(),
        meta: { note: 'accredito di sistema accredited ', _system_credit_jiaco_20: true, _system_credit_key: 'jiaco_20' }
      };
      ...
      try{
        ...
        console.error('add-credit-jiaco-20: update CUP9_EARNINGS failed', e);
      }
      ...
      try{
        ...
        console.warn('add-credit-jiaco-20: mirror to mock db failed', e);
      }
      ...
      console.error('add-credit-jiaco-20 local credit failed', e);
      try{ toastMessage('Accreditamento locale fallito', { type:'error' }); }catch(e){}
    }

  }catch(err){
    console.error('add-credit-jiaco-20 top-level error', err);
    try{ toastMessage('add-credit-jiaco-20 script error', { type:'error' }); }catch(e){}
  }
})();